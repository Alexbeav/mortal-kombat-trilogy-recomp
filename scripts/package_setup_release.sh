...
#
# Writes: dist/mortal-kombat-trilogy-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix mortal-kombat-trilogy \
  --exe-name Mortal_Kombat_Trilogy_Recompiled \
  --display-name "Mortal Kombat Trilogy Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Mortal Kombat Trilogy (USA) (Rev 1).cue" \
